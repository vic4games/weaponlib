...
public class Placer extends ItemBlock
{
    public Placer(Block block)
    {
        super(block);
        this.setMaxDamage(0);
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if (!itemstack.isEmpty() && player.canPlayerEdit(pos, facing, itemstack))
        {
            ...
            if (block == this.block)
            {
                int i = ((Integer)iblockstate.getValue(Debris.LAYERS)).intValue();

                if (i < 8)
                {
                    IBlockState iblockstate1 = iblockstate.withProperty(Debris.LAYERS, Integer.valueOf(i + 1));
    ...
    public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack)
    {
        ...
        return (!(state.getBlock() instanceof Debris) || ((Integer)state.getValue(Debris.LAYERS)) > 7) ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
    }
