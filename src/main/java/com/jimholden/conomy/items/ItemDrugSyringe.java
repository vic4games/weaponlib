...
import com.jimholden.conomy.medical.PainModifier;
import com.jimholden.conomy.medical.PainUtility;
import com.jimholden.conomy.medical.SystemDrug;
...
public class ItemDrugSyringe extends ItemFood implements IHasModel, IChemical {	
	
	
	public ItemDrugSyringe(int amount, float saturation, boolean isWolfFood, String name, String drugName, int drugType, float potency, double weight, boolean synthetic)
	{
	...
	*/
	...
	protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer player) {
		if(worldIn.isRemote) {
			DrugCache.addDrug(new Drug("adr", 600, 6000, 600, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.001F, 0.005F, 0.0F, 0.0F, 0.0F, 0.0F));
		} else {
			double app = PainUtility.getDrugApplicator(0.8, PainUtility.getPainFactor(player.getMaxHealth(), player.getHealth()), 50);
			IConscious cDat = player.getCapability(ConsciousProvider.CONSCIOUS, null);
			
			
			
			SystemDrug comStimOne = new SystemDrug(new PainModifier(app*cDat.getPainLevel()), 3000);
			comStimOne.addAbility("commieJuice", 30, 0);
			cDat.addDrug(comStimOne);
			...
			player.addPotionEffect(new PotionEffect(MobEffects.RESISTANCE, 600, 1, false, false));
		}
