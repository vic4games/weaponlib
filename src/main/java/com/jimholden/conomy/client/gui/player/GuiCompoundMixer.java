...
public class GuiCompoundMixer extends GuiContainer {
	
	private static final ResourceLocation TEXTURES = new ResourceLocation(Reference.MOD_ID + ":textures/gui/guichemlab.png");
	...
	public GuiCompoundMixer(EntityPlayer player, TileEntityCompoundMixer tileentity, MinecraftServer server) {
		super(new ContainerCompoundMixer(player.inventory, tileentity));
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if(tileentity.isMixing()) {
			this.drawTexturedModalRect(80, 15, 194, 0, 8, 8);
		} else {
			this.drawTexturedModalRect(80, 15, 194, 8, 8, 8);
		}
		
		
		int progress = (int) ((int) 19-(19*((float) this.tileentity.timer/this.tileentity.maxTime)));
		this.drawTexturedModalRect(33, 31+progress, 0, 167+progress, 66, 19);
