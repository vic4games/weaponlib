...
public class StockToast implements IToast
{
    ...
    private float tracker = 1.0F;
    private float speedProgress;
    private final boolean hasProgressBar;

    public StockToast(TutorialToast.Icons iconIn, ITextComponent titleComponent, @Nullable ITextComponent subtitleComponent, boolean drawProgressBar, float speedProgress)
    {
        this.icon = iconIn;
    ...
    public IToast.Visibility draw(GuiToast toastGui, long delta)
    {
        ...
        toastGui.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI((EntityLivingBase)null, new ItemStack(ModItems.STOCKTERMINAL), 8, 8);
        //this.icon.draw(toastGui, 6, 6);
        
        if(!(this.tracker <= 0)) {
        	this.tracker -= speedProgress;
        }
        
        setProgress(this.tracker);
        ...
        //System.out.println("Is prog: " + this.hasProgressBar);
